...
const TemplatesGallery = () => {
  ...
  const onTemplateClick = (title: string, initialContent: string) => {
    setIsCreating(true);
    create({ title, initialContent, orgId: organization!.id })
      .catch(() => toast.error(dict?.documents.createError))
      .then(documentId => {
        toast.success(dict?.documents.documentCreated);
  ...
  return (
    <div className="max-w-5xl mx-auto px-16 py-6 flex flex-col gap-y-4">
      <h3 className="font-medium">
        <TranslateText value="documents.createDocument" />
      </h3>
      <Carousel>
